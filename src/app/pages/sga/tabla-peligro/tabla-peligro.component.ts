...
import { Registro } from 'src/app/components/tabla-home/tabla-home.component';

export type SgaPictogram = 'llama' | 'peligro-salud' | 'exclamacion';
export interface SgaFicha {
  productName: string;
  supplier: string;
  displayVisibility: 'Publico' | 'Privado';
  dataSource: string;                  // ej: "FDS Proveedor"
  pictograms: SgaPictogram[];          // orden a mostrar
  signalWord: 'PELIGRO' | 'ATENCIÓN';
  hazardStatements: string[];          // H...
  precautionaryStatements: string[];   // P...
}
